...
gac() {
	...
	# Stage all changes (more complete than `git add .`)
	git add -A

	# Optional: detect pre-commit config
	...
	# No commit message -> open editor (ensure it blocks)
	if [[ -z "$msg" ]]; then
		...
		# Patch common GUI editors that need a wait flag
	...
	# Copy to clipboard (best effort, cross-platform-ish)
	if command -v pbcopy >/dev/null 2>&1; then
		...
		printf '%s' "$trimmed_msg" | xsel --clipboard --input
		echo "📋 Commit message copied to clipboard (xsel)."
	else
		echo "ℹ️  Clipboard tool not found; skipping copy."
	fi
